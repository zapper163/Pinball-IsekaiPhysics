...
void ModuleFonts::BlitText(int x, int y, int font_id, const char* text, int zoom, int r, int g, int b, int max, int down) const
{
	...
	for (uint i = 0; i < len; ++i)
	{
		...
		if (text[i] == '|') {
			...
			App->renderer->Blit(font->texture, x+4, y, &spriteRect, 1.0f, 0, INT_MAX, INT_MAX, r, g, b);
			x += spriteRect.w;
		}
		else {
			App->renderer->Blit(font->texture, x, y, &spriteRect, 1.0f, 0, INT_MAX, INT_MAX, r, g, b);
			x += spriteRect.w - 8;
		}
